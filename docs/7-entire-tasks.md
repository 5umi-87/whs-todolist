...
- [ ] **프로필 페이지**: 사용자 정보 조회 API 연동
- [ ] **프로필 수정 폼**: `react-hook-form`을 사용한 사용자 이름 및 비밀번호 수정 기능 구현
- [ ] **유효성 검사**: 비밀번호 변경 시 zod를 이용한 클라이언트 측 검증 구현

> ✅ **완료 조건**:
>
> - 프로필 페이지에서 현재 사용자 정보(이메일, 이름, 가입일)가 표시됨.
> - 사용자 이름 수정 및 비밀번호 변경 기능이 API와 연동되어 정상 동작함.
> - 유효성 검사가 정상적으로 작동하고 에러 메시지가 표시됨.
>
> 🔗 **의존성**: `Task FE-2`, `Task FE-3`, `Task FE-4`, `Task BE-3`

#### **Task FE-9: 반응형 디자인 및 다크모드 적용**
...
>
> 🔗 **의존성**: `Task FE-6`, `Task FE-7`, `Task FE-8`

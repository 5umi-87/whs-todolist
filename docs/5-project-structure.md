...
**설명**:
- API 호출은 service 레이어에서 처리
- 비즈니스 로직은 커스텀 훅 또는 서비스에서 처리
...
**설명**:
- 중복된 로직은 공통 유틸리티 함수로 추출
- 3번 이상 반복되는 코드는 함수/컴포넌트로 분리
- utils, hooks, services 디렉토리에 공통 로직 정리
...
**설명**:
- 할일 관련 로직은 todoStore에만
- 사용자 관련 로직은 authStore에만
- 각 컴포넌트는 하나의 역할만 수행
...
**레이어 역할**:
- **Presentation Layer**: 라우트 정의 및 요청/응답 처리
- **Business Logic Layer**: 비즈니스 로직 및 데이터 검증
- **Data Access Layer**: DB 액세스 (Prisma 사용)
...
**설명**:
- Repository는 Controller를 import하지 않음
- Service는 Controller를 import하지 않음
...
**설명**:
- 모듈 A가 B를 import하면, B는 A를 import하지 않음
- 공통 로직은 별도 모듈로 분리
- 순환 의존성 발생 시 리팩토링 필수
...
**설명**:
- Service는 구체적인 Repository 구현이 아닌 인터페이스에 의존
- 의존성은 생성자를 통해 주입
...
**상수 종류**:
- API 엔드포인트: `API_ENDPOINTS`
- 검증 규칙: `MIN_PASSWORD_LENGTH`, `MAX_TODO_TITLE_LENGTH`
- 상태 값: `TODO_STATUS`
...
**프론트엔드 환경 변수**:
- `REACT_APP_API_BASE_URL`: API 서버 URL
- `REACT_APP_ENV`: 환경 (development, production)

**백엔드 환경 변수**:
- `DATABASE_URL`: 데이터베이스 연결 URL
- `JWT_SECRET`: JWT 서명 키
- `JWT_ACCESS_TOKEN_EXPIRY`: Access Token 만료 시간
- `JWT_REFRESH_TOKEN_EXPIRY`: Refresh Token 만료 시간
- `PORT`: 서버 포트
- `NODE_ENV`: Node.js 환경
...
- Node.js crypto 모듈 사용: `require('crypto').randomBytes(64).toString('hex')`
...
**설명**:
- 프론트엔드는 자체 백엔드 API 호출
- 백엔드가 환경 변수로 관리된 API 키로 외부 API 호출
...
**원칙**:
- 함수형 컴포넌트 사용 (React 18 권장)
- Props Destructuring 사용
- 명확한 조건부 렌더링

**주요 패턴**:
- Early Return: 조건에 맞지 않으면 일찍 반환
- 삼항 연산자: 간단한 조건부 렌더링
- `&&` 연산자: Boolean 조건부 렌더링

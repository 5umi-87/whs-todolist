...
gh issue create \
  --title "[DB] 데이터베이스 스키마 및 테이블 생성" \
  --label "feature,database,complexity:medium" \
  --body "## Todo (해야할 일)
- [x] \`users\` 테이블 생성
- [x] \`todos\` 테이블 생성
- [x] \`holidays\` 테이블 생성
...
gh issue create \
  --title "[Backend] 백엔드 프로젝트 설정" \
  --label "feature,backend,setup,complexity:low" \
  --body "## Todo (해야할 일)
...
# Task BE-2: 핵심 미들웨어 구현
gh issue create \
  --title "[Backend] 핵심 미들웨어 구현" \
  --label "feature,backend,middleware,complexity:medium" \
  --body "## Todo (해야할 일)
...
# Task BE-3: 사용자 인증 API 구현
gh issue create \
  --title "[Backend] 사용자 인증 API 구현" \
  --label "feature,backend,auth,complexity:high" \
  --body "## Todo (해야할 일)
...
# Task BE-4: 할일(Todo) CRUD API 구현
gh issue create \
  --title "[Backend] 할일(Todo) CRUD API 구현" \
  --label "feature,backend,todo,complexity:high" \
  --body "## Todo (해야할 일)
...
# Task BE-5: 휴지통 API 구현
gh issue create \
  --title "[Backend] 휴지통 API 구현" \
  --label "feature,backend,trash,complexity:low" \
  --body "## Todo (해야할 일)
...
# Task BE-6: 국경일 API 구현
gh issue create \
  --title "[Backend] 국경일 API 구현" \
  --label "feature,backend,holiday,complexity:medium" \
  --body "## Todo (해야할 일)
...
gh issue create \
  --title "[Frontend] 프론트엔드 프로젝트 설정" \
  --label "feature,frontend,setup,complexity:low" \
  --body "## Todo (해야할 일)
...
# Task FE-2: 라우팅 및 레이아웃 구현
gh issue create \
  --title "[Frontend] 라우팅 및 레이아웃 구현" \
  --label "feature,frontend,routing,complexity:medium" \
  --body "## Todo (해야할 일)
...
# Task FE-3: 전역 상태 관리(Zustand) 및 API 서비스 설정
gh issue create \
  --title "[Frontend] 전역 상태 관리(Zustand) 및 API 서비스 설정" \
  --label "feature,frontend,state-management,complexity:high" \
  --body "## Todo (해야할 일)
...
# Task FE-4: 공통 컴포넌트 구현
gh issue create \
  --title "[Frontend] 공통 컴포넌트 구현" \
  --label "feature,frontend,components,complexity:medium" \
  --body "## Todo (해야할 일)
...
# Task FE-5: 사용자 인증 화면 구현
gh issue create \
  --title "[Frontend] 사용자 인증 화면 구현" \
  --label "feature,frontend,auth,complexity:medium" \
  --body "## Todo (해야할 일)
...
# Task FE-6: 할일 관리 화면 구현
gh issue create \
  --title "[Frontend] 할일 관리 화면 구현" \
  --label "feature,frontend,todo,complexity:high" \
  --body "## Todo (해야할 일)
...
# Task FE-7: 휴지통 화면 구현
gh issue create \
  --title "[Frontend] 휴지통 화면 구현" \
  --label "feature,frontend,trash,complexity:low" \
  --body "## Todo (해야할 일)
...
# Task FE-8: 반응형 디자인 및 다크모드 적용
gh issue create \
  --title "[Frontend] 반응형 디자인 및 다크모드 적용" \
  --label "feature,frontend,ui,complexity:medium" \
  --body "## Todo (해야할 일)
